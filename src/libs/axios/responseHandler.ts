...
import LoginRepositoryImpl from "../../repository/auth/login/login.repositoryImpl";
...
const ResponseHandler = async (error: AxiosError) => {
  ...
  if (response) {
    ...
    if (status === 401 && usingAccessToken !== undefined && usingRefreshToken !== undefined && !isRefreshing) {
      ...
      try {
        const { accessToken: newAccessToken } = await LoginRepositoryImpl.refreshAccessToken({
